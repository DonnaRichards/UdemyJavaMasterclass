...
NOTE: The method isPalindrome needs to be defined as public static like we have been doing
    ...
    public static boolean isPalindrome(int number) {
        int reverseNumber = 0;
        number = Math.abs(number);  // ensure using positive value, in case negative was passed in
        int workingNumber = number;
        while (workingNumber > 0) {
            int lastDigit = workingNumber % 10;  // extract last digit from number
            reverseNumber *= 10; // increase place value of reversed number
            reverseNumber += lastDigit; // add last digit of number to reversed number
            workingNumber /= 10; // remove last digit of number
        }
        return number == reverseNumber;
    }

    public static boolean isPalindrome_StringBuilder(int number) {
        StringBuilder str = new StringBuilder(Integer.toString(Math.abs(number)));
